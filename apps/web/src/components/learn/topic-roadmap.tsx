...
export function TopicRoadmap({ chapters }: { chapters: ChapterSummary[] }) {
  return (
    <section className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
      <div className="border-b border-border px-4 py-3">
        <p className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">DSA Topics</p>
        <h2 className="mt-1 text-lg font-semibold tracking-normal">Pick a topic to begin</h2>
      </div>
      ...
      <div className="grid gap-3 p-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
        {chapters.map((chapter, index) => {
          ...
          return (
            <Link
              key={chapter.slug}
              href={introduction?.href ?? `/learn/${chapter.slug}`}
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                ...
                <span className="mt-1.5 line-clamp-2 block text-xs leading-5 text-muted-foreground">{chapter.description}</span>
              </span>
