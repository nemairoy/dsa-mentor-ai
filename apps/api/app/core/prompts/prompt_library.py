...
class PromptLibrary:
    ...
    def _system_policy(self) -> str:
        return (
            ...
            "Be technically precise: verify algorithms, examples, edge cases, code syntax, stated complexity, "
            "and the supplied execution contract before answering. Never invent an API or claim code was executed."
    ...
    def _templates(self) -> dict[AiFeature, str]:
        return {
            ...
            AiFeature.explain_code: (
                "Explain what this code does in the context of {lesson_title}. Check its correctness first.\n"
                "Code:\n{code}\nLesson:\n{lesson_markdown}"
            ),
            AiFeature.line_by_line_code: (
                "Give a precise line-by-line explanation of this code after checking its syntax and logic.\nCode:\n{code}"
            ),
            AiFeature.convert_code: (
                "Convert this {source_language} code to {target_language}. Preserve behavior and complexity. "
                "Return complete code first, then notes.\nCode:\n{code}"
            ),
            ...
            AiFeature.follow_up: (
                "Answer the student's follow-up question using the lesson context. Verify examples and code before responding.\n"
                "Lesson: {lesson_title}\nContent:\n{lesson_markdown}\nQuestion: {question}"
            ),
        }
