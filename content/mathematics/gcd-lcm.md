...
## Goal
Learn Gcd Lcm in Mathematics with theory, example, code, explanation, complexity, and practice.

## Theory
Gcd Lcm is a focused part of Mathematics. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Mathematics: Gcd Lcm before writing code.
...
```python
def mathematics_gcd_lcm(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(mathematics_gcd_lcm([3, 1, 4, 1, 5]))
```
...
For Mathematics: Gcd Lcm, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
