...
## Goal
Learn Sieve Of Eratosthenes in Mathematics with theory, example, code, explanation, complexity, and practice.

## Theory
Sieve Of Eratosthenes is a focused part of Mathematics. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Mathematics: Sieve Of Eratosthenes before writing code.
...
```python
def mathematics_sieve_of_eratosthenes(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(mathematics_sieve_of_eratosthenes([3, 1, 4, 1, 5]))
```
...
For Mathematics: Sieve Of Eratosthenes, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
