...
## Goal
Learn Heap Sort in Heap with theory, example, code, explanation, complexity, and practice.

## Theory
Heap Sort is a focused part of Heap. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Heap: Heap Sort before writing code.
...
```python
def heap_heap_sort(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(heap_heap_sort([3, 1, 4, 1, 5]))
```
...
For Heap: Heap Sort, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
