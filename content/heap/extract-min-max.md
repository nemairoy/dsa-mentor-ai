...
## Goal
Learn Extract Min Max in Heap with theory, example, code, explanation, complexity, and practice.
...
```python
def heap_extract_min_max(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(heap_extract_min_max([3, 1, 4, 1, 5]))
```
