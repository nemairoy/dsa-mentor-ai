# Segment Tree: Min Max Sum Queries

## Goal
Learn Min Max Sum Queries in Segment Tree with theory, example, code, explanation, complexity, and practice.

## Theory
Min Max Sum Queries is a focused part of Segment Tree. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Segment Tree: Min Max Sum Queries before writing code.
...
```python
def segment_tree_min_max_sum_queries(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(segment_tree_min_max_sum_queries([3, 1, 4, 1, 5]))
```
...
For Segment Tree: Min Max Sum Queries, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
