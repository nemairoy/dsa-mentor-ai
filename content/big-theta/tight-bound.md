...
## Goal
Learn Tight Bound in Big Theta with theory, example, code, explanation, complexity, and practice.

## Theory
Tight Bound is a focused part of Big Theta. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Big Theta: Tight Bound before writing code.
...
```python
def big_theta_tight_bound(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(big_theta_tight_bound([3, 1, 4, 1, 5]))
```
...
For Big Theta: Tight Bound, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
