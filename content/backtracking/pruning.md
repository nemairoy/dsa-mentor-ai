...
## Goal
Learn Pruning in Backtracking with theory, example, code, explanation, complexity, and practice.

## Theory
Pruning is a focused part of Backtracking. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Backtracking: Pruning before writing code.
...
```python
def backtracking_pruning(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(backtracking_pruning([3, 1, 4, 1, 5]))
```
...
For Backtracking: Pruning, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
