# Backtracking: Choice Constraint Goal

## Goal
Learn Choice Constraint Goal in Backtracking with theory, example, code, explanation, complexity, and practice.

## Theory
Choice Constraint Goal is a focused part of Backtracking. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Backtracking: Choice Constraint Goal before writing code.
...
```python
def backtracking_choice_constraint_goal(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(backtracking_choice_constraint_goal([3, 1, 4, 1, 5]))
```
...
For Backtracking: Choice Constraint Goal, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
