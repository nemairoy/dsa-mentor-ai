...
## Goal
Learn Counting Sort in Sorting with theory, example, code, explanation, complexity, and practice.

## Theory
Counting Sort is a focused part of Sorting. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Sorting: Counting Sort before writing code.
...
```python
def sorting_counting_sort(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(sorting_counting_sort([3, 1, 4, 1, 5]))
```
...
For Sorting: Counting Sort, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
