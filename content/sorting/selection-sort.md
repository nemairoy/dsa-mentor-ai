...
## Goal
Learn Selection Sort in Sorting with theory, example, code, explanation, complexity, and practice.
...
```python
def sorting_selection_sort(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(sorting_selection_sort([3, 1, 4, 1, 5]))
```
