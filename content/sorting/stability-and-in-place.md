...
## Goal
Learn Stability And In Place in Sorting with theory, example, code, explanation, complexity, and practice.

## Theory
Stability And In Place is a focused part of Sorting. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Sorting: Stability And In Place before writing code.
...
```python
def sorting_stability_and_in_place(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(sorting_stability_and_in_place([3, 1, 4, 1, 5]))
```
...
For Sorting: Stability And In Place, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
