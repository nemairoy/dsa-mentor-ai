# Interview Preparation: Mock Interview Flow

## Goal
Learn Mock Interview Flow in Interview Preparation with theory, example, code, explanation, complexity, and practice.

## Theory
Mock Interview Flow is a focused part of Interview Preparation. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Interview Preparation: Mock Interview Flow before writing code.
...
```python
def interview_preparation_mock_interview_flow(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(interview_preparation_mock_interview_flow([3, 1, 4, 1, 5]))
```
...
For Interview Preparation: Mock Interview Flow, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
