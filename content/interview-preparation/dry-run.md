# Interview Preparation: Dry Run

## Goal
Learn Dry Run in Interview Preparation with theory, example, code, explanation, complexity, and practice.

## Theory
Dry Run is a focused part of Interview Preparation. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Interview Preparation: Dry Run before writing code.
...
```python
def interview_preparation_dry_run(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(interview_preparation_dry_run([3, 1, 4, 1, 5]))
```
...
For Interview Preparation: Dry Run, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
