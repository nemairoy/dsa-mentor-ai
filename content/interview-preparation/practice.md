...
## Goal
Learn Practice in Interview Preparation with theory, example, code, explanation, complexity, and practice.
...
```python
def interview_preparation_practice(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(interview_preparation_practice([3, 1, 4, 1, 5]))
```
