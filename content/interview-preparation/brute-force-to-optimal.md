# Interview Preparation: Brute Force To Optimal

## Goal
Learn Brute Force To Optimal in Interview Preparation with theory, example, code, explanation, complexity, and practice.

## Theory
Brute Force To Optimal is a focused part of Interview Preparation. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Interview Preparation: Brute Force To Optimal before writing code.
...
```python
def interview_preparation_brute_force_to_optimal(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(interview_preparation_brute_force_to_optimal([3, 1, 4, 1, 5]))
```
...
For Interview Preparation: Brute Force To Optimal, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
