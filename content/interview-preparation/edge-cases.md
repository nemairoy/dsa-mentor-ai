# Interview Preparation: Edge Cases

## Goal
Learn Edge Cases in Interview Preparation with theory, example, code, explanation, complexity, and practice.

## Theory
Edge Cases is a focused part of Interview Preparation. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Interview Preparation: Edge Cases before writing code.
...
```python
def interview_preparation_edge_cases(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(interview_preparation_edge_cases([3, 1, 4, 1, 5]))
```
...
For Interview Preparation: Edge Cases, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
