...
## Goal
Learn Examples in Big Omega with theory, example, code, explanation, complexity, and practice.

## Theory
Examples is a focused part of Big Omega. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Big Omega: Examples before writing code.
...
```python
def big_omega_examples(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(big_omega_examples([3, 1, 4, 1, 5]))
```
...
For Big Omega: Examples, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
