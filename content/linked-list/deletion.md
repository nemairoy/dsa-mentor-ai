...
## Goal
Learn Deletion in Linked List with theory, example, code, explanation, complexity, and practice.
...
```python
def linked_list_deletion(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(linked_list_deletion([3, 1, 4, 1, 5]))
```
