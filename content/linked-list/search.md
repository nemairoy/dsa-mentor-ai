...
## Goal
Learn Search in Linked List with theory, example, code, explanation, complexity, and practice.

## Theory
Search is a focused part of Linked List. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Linked List: Search before writing code.
...
```python
def linked_list_search(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(linked_list_search([3, 1, 4, 1, 5]))
```
...
For Linked List: Search, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
