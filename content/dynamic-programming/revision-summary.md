# Dynamic Programming: Revision Summary

## Goal
Learn Revision Summary in Dynamic Programming with theory, example, code, explanation, complexity, and practice.

## Theory
Revision Summary is a focused part of Dynamic Programming. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Dynamic Programming: Revision Summary before writing code.
...
```python
def dynamic_programming_revision_summary(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(dynamic_programming_revision_summary([3, 1, 4, 1, 5]))
```
...
For Dynamic Programming: Revision Summary, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
