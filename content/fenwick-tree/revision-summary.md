# Fenwick Tree: Revision Summary

## Goal
Learn Revision Summary in Fenwick Tree with theory, example, code, explanation, complexity, and practice.

## Theory
Revision Summary is a focused part of Fenwick Tree. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Fenwick Tree: Revision Summary before writing code.
...
```python
def fenwick_tree_revision_summary(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(fenwick_tree_revision_summary([3, 1, 4, 1, 5]))
```
...
For Fenwick Tree: Revision Summary, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
