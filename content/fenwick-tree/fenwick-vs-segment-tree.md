# Fenwick Tree: Fenwick Vs Segment Tree

## Goal
Learn Fenwick Vs Segment Tree in Fenwick Tree with theory, example, code, explanation, complexity, and practice.

## Theory
Fenwick Vs Segment Tree is a focused part of Fenwick Tree. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Fenwick Tree: Fenwick Vs Segment Tree before writing code.
...
```python
def fenwick_tree_fenwick_vs_segment_tree(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(fenwick_tree_fenwick_vs_segment_tree([3, 1, 4, 1, 5]))
```
...
For Fenwick Tree: Fenwick Vs Segment Tree, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
