# Greedy: Revision Summary

## Goal
Learn Revision Summary in Greedy with theory, example, code, explanation, complexity, and practice.

## Theory
Revision Summary is a focused part of Greedy. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Greedy: Revision Summary before writing code.
...
```python
def greedy_revision_summary(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(greedy_revision_summary([3, 1, 4, 1, 5]))
```
...
For Greedy: Revision Summary, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
