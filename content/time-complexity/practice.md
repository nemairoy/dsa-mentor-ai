...
## Goal
Learn Practice in Time Complexity with theory, example, code, explanation, complexity, and practice.

## Theory
Practice is a focused part of Time Complexity. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Time Complexity: Practice before writing code.
...
```python
def time_complexity_practice(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(time_complexity_practice([3, 1, 4, 1, 5]))
```
...
For Time Complexity: Practice, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
