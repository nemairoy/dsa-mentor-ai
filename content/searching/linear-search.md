...
## Goal
Learn Linear Search in Searching with theory, example, code, explanation, complexity, and practice.

## Theory
Linear Search is a focused part of Searching. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Searching: Linear Search before writing code.
...
```python
def searching_linear_search(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(searching_linear_search([3, 1, 4, 1, 5]))
```
...
For Searching: Linear Search, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
