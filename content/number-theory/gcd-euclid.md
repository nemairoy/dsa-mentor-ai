...
## Goal
Learn Gcd Euclid in Number Theory with theory, example, code, explanation, complexity, and practice.

## Theory
Gcd Euclid is a focused part of Number Theory. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Number Theory: Gcd Euclid before writing code.
...
```python
def number_theory_gcd_euclid(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(number_theory_gcd_euclid([3, 1, 4, 1, 5]))
```
...
For Number Theory: Gcd Euclid, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
