...
## Goal
Learn Tail Recursion in Recursion with theory, example, code, explanation, complexity, and practice.

## Theory
Tail Recursion is a focused part of Recursion. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Recursion: Tail Recursion before writing code.
...
```python
def recursion_tail_recursion(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(recursion_tail_recursion([3, 1, 4, 1, 5]))
```
...
For Recursion: Tail Recursion, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
