...
## Goal
Learn Cycle Detection in Disjoint Set Union with theory, example, code, explanation, complexity, and practice.
...
```python
def disjoint_set_union_cycle_detection(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(disjoint_set_union_cycle_detection([3, 1, 4, 1, 5]))
```
