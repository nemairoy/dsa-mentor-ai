...
Study the core principles, invariants, and correctness ideas. The goal is to understand binary search tree from first principles and connect it to practical problem solving.
...
```python
def binary_search_tree_theory(items):
    """Reference scaffold for Binary Search Tree: Theory."""
    result = []
...
print(binary_search_tree_theory([3, 1, 4, 1, 5]))
```
