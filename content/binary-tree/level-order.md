...
## Goal
Learn Level Order in Binary Tree with theory, example, code, explanation, complexity, and practice.
...
```python
def binary_tree_level_order(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(binary_tree_level_order([3, 1, 4, 1, 5]))
```
