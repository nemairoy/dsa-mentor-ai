...
## Goal
Learn Monotonic Stack in Stack with theory, example, code, explanation, complexity, and practice.

## Theory
Monotonic Stack is a focused part of Stack. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Stack: Monotonic Stack before writing code.
...
```python
def stack_monotonic_stack(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(stack_monotonic_stack([3, 1, 4, 1, 5]))
```
...
For Stack: Monotonic Stack, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
