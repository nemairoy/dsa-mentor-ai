# Competitive Programming: Debugging Strategy

## Goal
Learn Debugging Strategy in Competitive Programming with theory, example, code, explanation, complexity, and practice.

## Theory
Debugging Strategy is a focused part of Competitive Programming. Learn the rule, the state it changes, and the invariant that makes the operation correct.

## Step-by-step example
Use this trace to understand Competitive Programming: Debugging Strategy before writing code.
...
```python
def competitive_programming_debugging_strategy(items):
    result = []
    for index, value in enumerate(items):
        result.append((index, value))
    return result

print(competitive_programming_debugging_strategy([3, 1, 4, 1, 5]))
```
...
For Competitive Programming: Debugging Strategy, focus on the state before and after each step. A correct explanation should say what changed, why it changed, and why the final answer follows from the rule.
